...
Public NotInheritable Class Runner
    ...
    Private Shared _processId As Integer = 0
    ...
    Public Sub New(small As Integer, medium As Integer, large As Integer, strategy As IStrategy)
        ...
        For i As Integer = 0 To small - 1
            ...
            _processId += 1
            _processLoad.Add(New Process(_processId, random.[Next](CInt(BurstTime.SmallMin), CInt(BurstTime.SmallMax)), random.[Next](0, total)) With {
                    .Priority = DirectCast(random.[Next](0, 3), Priority)
            })
        Next
        For i As Integer = 0 To medium - 1
            _processId += 1
            _processLoad.Add(New Process(_processId, random.[Next](CInt(BurstTime.MediumMin), CInt(BurstTime.MediumMax)), random.[Next](0, total)) With {
                    .Priority = DirectCast(random.[Next](0, 3), Priority)
            })
        Next
        For i As Integer = 0 To large - 1
            _processId += 1
            _processLoad.Add(New Process(_processId, random.[Next](CInt(BurstTime.LargeMin), CInt(BurstTime.LargeMax)), random.[Next](0, total)) With {
                    .Priority = DirectCast(random.[Next](0, 3), Priority)
            })
        Next
